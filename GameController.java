...
public class GameController{
  ...
  public static void main(String[] args) { 
    ...
    board.drawTiles(); // Tiles only

    // Starting player
    String playersTurn = Board.red; // red is 0, black is 1
    ...
    while(true){
    	...
    	if( mousePressedThisFrame ){
    		...
    		if( mouseX > 0 && mouseX < 1.0 && mouseY > 0 && mouseY < 1.0){
	    		...
	    		if(!isSelected){
	    			...
	    			String color = board.getTileColor(yInt, xInt);
	    			// if its the red players turn
	    			if (playersTurn.equals(Board.red)){
	    				// and a red tile is selected
	    				if(color.equals(Board.red)){
	    					...
	    					System.out.println("Not a valid tile to select, must be Red");
	    				}
				...
				int redCount = board.countTiles(Board.red);
    	...
    	// pause and redraw. Needed to listen for input. 
    	StdDraw.show(50);
    }
  }
}